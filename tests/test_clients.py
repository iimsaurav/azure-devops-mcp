...
class TestPipelinesClient:
    ...
    @patch(
        "azure_devops_mcp.clients.pipelines._get_org_url",
...
class TestGitClient:
    ...
    @patch(
        "azure_devops_mcp.clients.git._get_org_url",
        return_value="https://dev.azure.com/myorg",
    )
    ...
    def test_compare_branches_caps_changes(self, mock_url, mock_api):
        ...
        changes = [
            {"changeType": "edit", "item": {"path": f"/file{i}.py", "isFolder": False}}
            for i in range(150)
        ]
        mock_api.return_value = {
            "aheadCount": 5,
...
class TestBoardsClient:
    ...
    @patch("azure_devops_mcp.clients.boards._api")
    @patch(
        "azure_devops_mcp.clients.boards._get_org_url",
        return_value="https://dev.azure.com/myorg",
    )
    def test_update_work_item_uses_add_patch_op_for_fields(self, mock_url, mock_api):
        mock_api.return_value = {
            "id": 101,
        ...
        from azure_devops_mcp.clients.boards import update_work_item

        result = update_work_item("MyProject", 101, fields={"System.Title": "Updated"})

        assert result["id"] == 101
        _, kwargs = mock_api.call_args
        patch_doc = kwargs["json_body"]
        assert patch_doc[0]["op"] == "add"
        assert patch_doc[0]["path"] == "/fields/System.Title"
    ...
    def test_delete_work_item_handles_empty_delete_response(self, mock_url, mock_api):
        mock_api.return_value = {"text": ""}
        from azure_devops_mcp.clients.boards import delete_work_item

        result = delete_work_item("MyProject", 555)

        assert result["id"] == 555
        assert result["message"] == "Work item deleted"
...
class TestIdentityFormatting:
    ...
    def test_format_dict_with_display_name(self):
        ...
        assert (
            _format_identity(
                {"displayName": "John Doe", "uniqueName": "john@example.com"}
            )
            == "John Doe"
        )
    ...
    def test_format_dict_with_unique_name_only(self):
        ...
        assert (
            _format_identity({"uniqueName": "john@example.com"}) == "john@example.com"
        )
